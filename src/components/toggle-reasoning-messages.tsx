...
const ReasoningMessageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
...
function useReasoningMessage() {
    ...
    if (!context) {
        throw new Error('useReasoningMessage must be used within a ReasoningMessageProvider.');
    }
    return context;
}
    ...
    return (
        <div className="flex w-max px-2 h-full">
            <div className="flex items-center gap-2 text-xs">
                ...
                <Switch id="show-reasoning-messages" checked={isEnabled} onCheckedChange={setIsEnabled} />
            </div>
        </div>
    )
}
