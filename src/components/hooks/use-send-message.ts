...
export function useSendMessage() {
    ...
    async function sendMessage(agentId: string, text: string) {
        ...
        try {
            queryClient.setQueriesData<AppMessage[]>({
                ...
                return [
                    ...data,
                    { id: 'new_' + Date.now(), date: Date.now(), message: text, messageType: MessageType.USER_MESSAGE },
                    { id: 'deleteme_', date: Date.now(), message: 'loading...', messageType: MessageType.TOOL_CALL_MESSAGE }
                ]
            })
    ...
    return useMutation({
        mutationFn: ({ agentId, text }: { agentId: string; text: string }) => sendMessage(agentId, text),
    });
}